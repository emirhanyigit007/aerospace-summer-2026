...
final_pitch_list = zeros(length(theta_ref_deg_list), 1);
max_pitch_list = zeros(length(theta_ref_deg_list), 1);
max_q_list = zeros(length(theta_ref_deg_list), 1);
max_elevator_list = zeros(length(theta_ref_deg_list), 1);
steady_state_error_list = zeros(length(theta_ref_deg_list), 1);
overshoot_list = zeros(length(theta_ref_deg_list), 1);
settling_time_list = zeros(length(theta_ref_deg_list), 1);
...
xlabel('Time [s]');
ylabel('\theta [deg]');
title('Pitch Command Tracking Response');

%% Run Simulations

for i = 1:length(theta_ref_deg_list)

    theta_ref_deg = theta_ref_deg_list(i);
...
for i = 1:length(theta_ref_deg_list)
    yline(theta_ref_deg_list(i), '--', ...
        ['Target ', num2str(theta_ref_deg_list(i)), ' deg'], ...
        'LineWidth', 1.0);
end

legend('5 deg command', '10 deg command', '15 deg command', ...
    'Target 5 deg', 'Target 10 deg', 'Target 15 deg', ...
    'Location', 'best');

sgtitle('UAV Pitch Control - Multiple Command Tracking Test');

%% Print Results

fprintf('UAV Pitch Command Tracking Results\n');
fprintf('---------------------------------------------------------------------------------------------\n');
fprintf('%-12s %-12s %-12s %-12s %-14s %-12s %-12s\n', ...
    'Command', 'Final', 'Max Pitch', 'Max q', 'Max Elevator', 'Overshoot', 'Settling');
fprintf('---------------------------------------------------------------------------------------------\n');

for i = 1:length(theta_ref_deg_list)
    fprintf('%-12.2f %-12.2f %-12.2f %-12.2f %-14.2f %-12.2f %-12.2f\n', ...
        theta_ref_deg_list(i), ...
        final_pitch_list(i), ...
        max_pitch_list(i), ...
        max_q_list(i), ...
        max_elevator_list(i), ...
...
saveas(gcf, fullfile(results_folder, 'uav_pitch_command_tracking.png'));
...
writetable(results_table, fullfile(results_folder, 'uav_pitch_command_tracking_results.csv'));
